...
class FabCircularMenuState extends State<FabCircularMenu>
  ...
  Widget _applyTransformations(Widget child, int index) {
    ...
    final angle = vector.radians(150.0 / (widget.children.length-1) * (index-1) + angleFix);

    double turns = widget.children.length == 3 ? 45.0 : widget.children.length == 2 ? 118.0 : 21.0;
      ...
      child: Transform(
          transform: Matrix4.translationValues(
              (-(_ringDiameter! / 2) * cos(angle) +
                  ...
                  _directionX,
              (-(_ringDiameter! / 1.9) * sin(angle) +
                  (_ringWidth! / 1.9 * sin(angle))) *
                  _directionY,
              0.0),
          alignment: FractionalOffset.center,
          child: Material(
