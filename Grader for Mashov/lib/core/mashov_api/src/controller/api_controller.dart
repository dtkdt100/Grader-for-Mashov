...
import '../../../../utilities/phone/download_utilities.dart';
...
//    try {
  ...
  Future<List<BehaveCounter>> getEventsCounter(String userId,
      [bool removeInClass = false]) async {
    ...
    for (int i = 0; i < groups.length; i++) {
      ...
      var totalEvents = behaves!
          .where((element) =>
              element.groupId == groups[i].id &&
              (removeInClass ? element.eventCode != 135 : true))
          .toList();

      if (totalEvents.isNotEmpty) {
        for (int j = 0; j < totalEvents.length; j++) {
          BehaveEvent behaveEvent = totalEvents.elementAt(j);
          if (!(removeInClass && behaveEvent.eventCode == 135)) {
            events.add(ShortBehaveEvent(
              ...
              eventCode: behaveEvent.eventCode,
            ));
          }
        }
      }
