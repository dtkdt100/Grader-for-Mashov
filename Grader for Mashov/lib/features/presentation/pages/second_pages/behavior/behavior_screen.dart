...
class _BehaviorScreenState extends BaseScreen<BehaviorScreen> {
  ...
  Widget? get floatingActionButton => CustomFabCircularMenu(
      items: page == 0
          ? itemsForAll()
          : itemsForSortSubject(
              page == 1 ? behavesForSubjects! : behavesForEvents!));
  ...
  Widget bodyAll() {
    /// TODO: 135 for נוחכות בשיעור
    return Column(
      children: List.generate(events!.length, (index) {
        if (_sortMehtod == SortMehtod.yesEvents &&
            events![index].justificationId > -1) {
          return const SizedBox();
        } else if (_sortMehtod == SortMehtod.noEvents &&
            !(events![index].justificationId > -1)) {
          return const SizedBox();
        }
  ...
  @override
  void selected(int i) {
    if (i == threeDotsLabels.length - 1) {
      ChangeAvgZerosDialog().showWithAnimation(context, value:
      InfoForChange('הסרת נוחכות בשיעור:', 'האם ברצונך להסיר מחישוב אירועי התנהגות את האירוע התנהגות - נוכחות בשיעור?')
      ).then((value) {
        if (value != null) {
          SharedPreferencesUtilities.setInClass(value == true);
          events = [];
          noEvents = 0;
          yesEvents = 0;
          behavesForSubjects = [];
