...
import '../phone/shared_preferences_utilities.dart';
...
class MashovUtilities {
  ...
  static Future<HomePageData?> getHomePageData(VoidCallback callback) async {
    ...
    if (SharedPreferencesUtilities.homePageCards![3] == true) {
      homePageData.infoPlayer =
      await AvgGameController(MashovUtilities.loginData!.data).getInfo();
    }
    callback();
  ...
  static Future<Map<String, dynamic>> getGrades({int? max}) async {
    ...
    if (SharedPreferencesUtilities.lastAvg == null) {
      SharedPreferencesUtilities.setLastAvg(double.parse(avg));
    }
  ...
  static Future<Map<String, dynamic>> getBehaves() async {
    ...
    if (SharedPreferencesUtilities.removeInClass) {
      events.removeWhere((element) => element.eventCode == 135);
    }
  ...
  static Future<Map<String, dynamic>> getBehavesCount() async {
    var hours = await _controller.getEventsCounter(_getUserId(), SharedPreferencesUtilities.removeInClass);
